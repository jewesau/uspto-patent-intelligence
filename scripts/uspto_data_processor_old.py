...
class USPTODataProcessor:
    ...
    raw_df = self.load_uspto_data_from_zip()
    if raw_df is None:
        print("Failed to load USPTO data from zip file")
        return None
    
    print(f"Loaded {len(raw_df[0])} raw patent records")
    
    # Merge patent data
    merged_df = self._merge_patent_data(*raw_df)
    ...
    print(f"Loaded {len(raw_df)} raw patent records")
